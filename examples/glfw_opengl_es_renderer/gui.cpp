...
constexpr auto parameters_file = "parameters.json";
	...
	: m_model_files_regex("^.*\.(dae|obj|OBJ|fbx|glb|gltf)$")
{
	...
	{
		model_parameters = load_parameters(parameters_file);
	}
...
gui::~gui()
{
	...
	save_parameters(parameters_file, model_parameters);
}
...
void gui::render_model_options(float delta_time)
{
	...
	if (ImGui::CollapsingHeader("Point light"))
	{
		...
		ImGui::SliderFloat("Linear##pl", &model_parameters.point_light.attenuation.linear, 0.0014f, 0.07f);
		ImGui::SliderFloat("Quadratic##pl", &model_parameters.point_light.attenuation.quadratic, 0.000007f, 2.f);
	...
	if (ImGui::CollapsingHeader("Spot light"))
	{
		...
		ImGui::SliderFloat("Linear##sl", &model_parameters.spot_light.attenuation.linear, 0.0014f, 0.07f);
		ImGui::SliderFloat("Quadratic##sl", &model_parameters.spot_light.attenuation.quadratic, 0.000007f, 2.f);
